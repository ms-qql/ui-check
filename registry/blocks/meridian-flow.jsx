...
/*
 * meridian-flow — vierstufiger Ablauf (Steps 01–04, erster hervorgehoben)
 * neben einem stickenden Watch-/Device-Mockup (SEV-Karte, Budget-Burn-Meter).
 * Section-Typ: steps.  Keine Bild-Slots (Device ist code-gerendertes UI).
 * Props: { data } — Felder: kicker, title[], steps[], device{}, nowShowing, step.
 */
  ...
  return (
    <div className="rounded-2xl border border-paper/12 bg-ink-soft/50 p-6">
      <div className="mx-auto max-w-[300px] rounded-[2.5rem] bg-ink p-3 ring-1 ring-paper/15">
        <div className="rounded-[2rem] bg-ink px-5 py-6 ring-1 ring-paper/10">
          ...
          <div className="mt-6 text-center">
            ...
            <div className="mt-6 flex items-end justify-center gap-1">
              <span className="font-display text-5xl font-light leading-none text-paper">{d.value}</span>
            ...
            <div className="mt-2 h-1 overflow-hidden rounded-full bg-paper/10">
              <div className="h-full rounded-full bg-accent" style={{ width: `${(d.burn || 0) * 100}%` }} />
            </div>
          </div>
...
export default function MeridianFlow({ data = {} }) {
  const { id = "flow", kicker, title = [], steps = [], device = {}, nowShowing, step } = data;
  return (
          ...
          <div>
            {steps.map((s, i) => (
              <div key={s.num} className="border-t border-paper/12 py-8 first:border-t-0">
                ...
                <h3 className={`font-display mt-3 text-2xl font-medium md:text-3xl ${i === 0 ? "border-l-2 border-paper pl-4 text-paper" : "text-muted"}`}>
                  {s.title}
                </h3>
