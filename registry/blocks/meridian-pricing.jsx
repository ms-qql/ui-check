...
import { Button, Barcode } from "../lib/meridian-ui.jsx";
import { cn } from "../lib/cn.js";

/*
 * meridian-pricing — drei Preis-Tickets im Beleg-/Receipt-Stil (Mono, gestrichelte
 * Divider, Perforationskerben, Faux-Barcode; mittleres invertiert + hervorgehoben).
 * Section-Typ: pricing.  Keine Bild-Slots.
 * Props: { data } — Felder: bar, title[], foot, plans[].
 */
function Ticket({ p }) {
  ...
  return (
    <div className={cn("relative rounded-xl p-7 font-mono text-sm", light ? "bg-paper text-ink shadow-2xl lg:-my-6 lg:scale-[1.04]" : "border border-paper/12 bg-ink-soft/40 text-paper")}>
      ...
      <h3 className={cn("font-display text-xl leading-snug", strong)}>{p.blurb}</h3>
      ...
      <div className="flex items-baseline justify-between">
        ...
        <span className="flex items-baseline gap-1">
          <span className={cn("font-display text-3xl", strong)}>{p.total}</span>
...
export default function MeridianPricing({ data = {} }) {
  const { id = "pricing", bar, title = [], foot, plans = [] } = data;
  return (
