...
import { DispatchBar, Slot } from "../lib/meridian-ui.jsx";

/*
 * meridian-testimonials — Dispatch-Bar + zweifarbige Headline, horizontal
 * scrollende Portrait-Zitat-Karten (Editorial-„Frames") mit Pagination.
 * Section-Typ: testimonials.  Bild-Slots: pro Item ein Portrait (item.slot).
 * Props: { data } — Felder: barLeft/Center/Right, title[], items[].
 */
export default function MeridianTestimonials({ data = {} }) {
  const { id = "testimonials", barLeft, barCenter, barRight, title = [], items = [] } = data;
  return (
    <section id={id} className="relative overflow-hidden py-4">
      ...
      <div className="container-x mt-14 flex snap-x gap-6 overflow-x-auto pb-4">
        {items.map((it, i) => (
          <figure key={i} className="flex w-[min(28rem,85vw)] shrink-0 snap-start flex-col overflow-hidden rounded-xl border border-paper/10 bg-ink-soft/40">
            <div className="relative">
              <Slot id={it.slot || `testimonial-${i + 1}`} className="aspect-[4/5] w-full" />
            ...
            <div className="flex flex-1 flex-col p-7">
              <span className="font-display text-3xl leading-none text-paper/30">&ldquo;</span>
