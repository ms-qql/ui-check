...
/*
 * verdict-cases — Sticky-Stack aus Fall-Karten (abwechselnd Bild links/rechts).
 * Section-Typ: cases.  Bild-Slots: case-01 … case-04.
 * Props: { data } aus content.json (Sektion mit type "cases").
 */
  ...
  return (
    <article className="overflow-hidden rounded-3xl border border-line bg-paper shadow-[0_20px_60px_-25px_rgba(0,0,0,0.25)]">
      <div className="grid md:grid-cols-2">
        <Slot
          id={`case-${c.num}`}
          className={`min-h-[280px] md:min-h-[520px] ${imageLeft ? "md:order-1" : "md:order-2"}`}
        />
...
export default function VerdictCases({ data = {} }) {
  const { id = "cases", eyebrow = "Featured Case Studies", title, body, cta, items = [] } = data;
      ...
      <div className="container-x">
        <div className="grid gap-x-8 gap-y-6 lg:grid-cols-12">
          ...
          <div className="lg:col-span-9">
            <h2 className="display max-w-xl text-[clamp(2rem,4.6vw,3.5rem)]">{title}</h2>
            <p className="mt-6 max-w-lg text-[1.05rem] leading-relaxed text-muted">{body}</p>
          </div>
        </div>
