  ...
  return (
    <span className={cn("inline-flex items-center gap-2.5", className)}>
      ...
      <span className="text-lg font-medium tracking-tight" style={{ fontFamily: "var(--font-display)" }}>
        {word}
        <sup className="ml-0.5 text-[0.55em] align-super opacity-60">®</sup>
      </span>
    </span>
  );
}
 ...
 * variant: primary (heller Pill, dunkler Text, Glow) · invert (dunkler Pill) ·
 *          quiet (nur Text + Pfeil) · outline (Rahmen).
 */
...
/*
 * Dispatch-Strip: die wiederkehrende Kopf-Metazeile einer Sektion —
 * drei Mono-Labels (links mit ●-Punkt, mittig, rechts) über einer Haarlinie.
 */
...
/*
 * Spec-Card: bordierte Mono-Datenkarte (§ MRD / 01 … Zeilen mit Label / Wert).
 * rows: [{ k, v }]  ·  head: { title, tag }
 */
