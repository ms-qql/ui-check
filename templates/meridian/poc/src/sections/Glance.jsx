...
import { Slot, SpecCard } from "../lib/ui.jsx";

export default function Glance({ data = {} }) {
  const { kicker, title, spec, rows = [], footL, footR } = data;
  return (
    <section id="glance" className="section-padding">
        ...
        <div className="mt-16 grid gap-10 lg:grid-cols-2">
          ...
          <div>
            <div className="relative aspect-square">
              ...
              <Slot id="glance-visual" className="h-full w-full ring-1 ring-paper/15">
                <span className="mono-label absolute inset-0 grid place-items-center text-paper/70">Surface</span>
              </Slot>
            </div>
          ...
          <div>
            {rows.map((r) => (
              <div key={r.key} className="grid grid-cols-[2.5rem_1fr] gap-4 border-t border-paper/12 py-6 first:border-t-0 sm:grid-cols-[2.5rem_8rem_1fr]">
                <div className="text-2xl font-medium text-paper/80" style={{ fontFamily: "var(--font-display)" }}>{r.key}.</div>
