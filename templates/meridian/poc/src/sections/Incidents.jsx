...
export default function Incidents({ data = {} }) {
  const { barLeft, barCenter, barRight, titleA = [], titleB = [], brand, stepLabel, body, cols = [], rows = [], footL, footC, footR } = data;
  return (
    <section id="incidents" className="relative isolate overflow-hidden">
      <DispatchBar left={barLeft} center={barCenter} right={barRight} />

      <div className="container-x pt-16">
        {/* Asymmetrische Doppel-Headline */}
        ...
        <div className="mt-14">
          ...
          {rows.map((r, i) => (
            <div key={i} className={cn("grid grid-cols-[5rem_3rem_5rem_3rem_1fr_auto] items-center gap-4 border-b border-paper/8 py-4", r.peak && "bg-paper/[0.04]")}>
              ...
              <Spark peak={r.peak} />
              <span className={cn("text-2xl font-light", r.peak ? "text-accent" : "text-paper")} style={{ fontFamily: "var(--font-display)" }}>{r.mag}</span>
