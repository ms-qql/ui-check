...
function PhoneMock({ d }) {
  return (
    <div className="mx-auto w-[280px] rounded-[3rem] bg-ink p-3 shadow-2xl ring-1 ring-paper/20">
      <div className="relative overflow-hidden rounded-[2.4rem]" style={{ background: "linear-gradient(180deg,#efe9dd,#e4dccb)" }}>
        {/* Dynamic Island */}
        <div className="absolute left-1/2 top-3 z-10 flex -translate-x-1/2 items-center gap-2 rounded-full bg-black px-3 py-1.5">
        ...
        <div className="px-6 pb-6 pt-16 text-center text-ink">
          ...
          <div className="text-7xl font-light tracking-tight" style={{ fontFamily: "var(--font-display)" }}>{d.time}</div>
        </div>
        {/* Notification */}
        <div className="mx-3 mb-3 rounded-2xl bg-white/80 p-3 backdrop-blur">
...
export default function Island({ data = {} }) {
  const { kicker, title = [], nowCycling, subtitle = [], device, activities = [], body } = data;
  return (
    <section id="island" className="section-padding">
      <div className="container-x">
        <div className="relative overflow-hidden rounded-3xl p-1" style={{ background: "linear-gradient(135deg,rgba(255,255,255,.25),rgba(255,255,255,.03),rgba(255,255,255,.18))" }}>
